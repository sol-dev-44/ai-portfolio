...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel, Field
import tiktoken
from typing import List, Dict
...
# === CORS – dynamic + Vercel preview safe ===
origins = [
    "http://localhost:3000",                                          # dev
]

# Optional explicit production URL (set in Railway)
...
app.add_middleware(
    ...
    # Allow ANY Vercel preview URL (git-*, feature-*, etc.)
...
# ------------------------------------------------------------------ #
# (Everything else is unchanged – only CORS block was updated)
# ------------------------------------------------------------------ #
